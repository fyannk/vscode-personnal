...
export async function installCodeServer(
    ...
    // Code Personal always supplies its matching server archive in product.json.
    // Never silently substitute another distribution when that configuration is absent.
    const serverDownloadUrlTemplateFinal = serverDownloadUrlTemplate || vscodeServerConfig.serverDownloadUrlTemplate;
    if (!serverDownloadUrlTemplateFinal) {
        throw new ServerInstallError('No Code Personal server download URL is configured. Reinstall Code Personal or set remote.SSH.serverDownloadUrlTemplate.');
    }
